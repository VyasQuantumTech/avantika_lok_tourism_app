...
class _ProfileEditPageState extends State<ProfileEditPage> {
  ...
  void _bind(ProfileSnapshot profile) {
    ...
    _providerLegalName.text =
        provider?.legalName ?? '';

    _providerDisplayName.text =
        provider?.displayName ?? '';

    _providerPhone.text =
        provider?.phone ?? '';

    _providerAddress1.text =
        provider?.addressLine1 ?? '';

    _providerAddress2.text =
    ...
    try {
      await getIt<UpdateMyProfile>()(
        fields: <String, dynamic>{
          ...
          'lastName':
          _nullable(_lastName.text),
          'phone':
          _nullable(_phone.text),
          'gender': _gender,
          'dateOfBirth':
          _nullable(_dob.text),
          'birthTime':
          _nullable(_birthTime.text),
          'placeOfBirth':
          _nullable(_placeOfBirth.text),
          'addressLine1':
          _nullable(_address1.text),
          'addressLine2':
          _nullable(_address2.text),
          'city':
          _nullable(_city.text),
          'state':
          _nullable(_state.text),
          'postalCode':
          _nullable(_postal.text),
          'countryCode':
          loaded.personalProfile
              ?.countryCode ??
              'IN',
        },
      );
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.providerMode &&
      ...
      return const Scaffold(
        body: Center(
          child: Text(
            'Provider profile editing is only available in provider flavor.',
          ),
        ),
      );
    }
    ...
    if (!widget.providerMode &&
      ...
      return const Scaffold(
        body: Center(
          child: Text(
            'Customer profile editing is unavailable in provider flavor.',
          ),
        ),
      );
    }
